...
public class ServerNode extends Node {
	...
	public ServerNode parent = null; // the parent in the tree, null if this node is the root
	...
	public void handleMessages(Inbox inbox) {			
		...
		{
			if (coloringRoundCounter <= ServerNode.ROUNDS_TO_FINISH_COLORING) {
				// Color the graph
				six_Vcol(inbox);		
	...
	public void preStep() {
		...
		if (!this.hasStartedColoring) {
			...
			colorBinaryCurrentMaxLength = ServerNode.MAX_BINARY_LENGTH;
			// TODO remove
	...
	{
		ServerNode.ROUNDS_TO_FINISH_COLORING = logStar(maxN);
		
		// Number of bits in the maxColorValue = log_2 of it, rounded up
		int maxColorValue = maxN + ServerNode.FINAL_NUM_OF_COLORS;
		ServerNode.MAX_BINARY_LENGTH = (int)Math.ceil(Math.log(maxColorValue) / Math.log(2));
	}
