...
public class ClientNode extends Node {
	...
	public void handleMessages(Inbox inbox) {			
		...
		{
			if (coloringRoundCounter <= ClientNode.ROUNDS_TO_FINISH_COLORING) {
				// Color the graph
				six_Vcol(inbox);		
	...
	public void preStep() {
		...
		if (!this.hasStartedColoring) {
			...
			colorBinaryCurrentMaxLength = ClientNode.MAX_BINARY_LENGTH;
			// TODO remove
	...
	{
		ClientNode.ROUNDS_TO_FINISH_COLORING = logStar(maxN);
		
		// Number of bits in the maxColorValue = log_2 of it, rounded up
		int maxColorValue = maxN + ClientNode.FINAL_NUM_OF_COLORS;
		ClientNode.MAX_BINARY_LENGTH = (int)Math.ceil(Math.log(maxColorValue) / Math.log(2));
	}
